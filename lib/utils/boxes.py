...
def onedim_bbox_transform(boxes, deltas, weights = (1.0, 1.0, 1.0, 1.0)):
	"""Forward transform that maps proposal boxes to predicted ground-truth
	boxes using bounding-box regression deltas. See bbox_transform_inv for a
	description of the weights argument.
	"""
	...
	pred_ctr_x = dx[:, np.newaxis] * widths[:, np.newaxis] + ctr_x[:, np.newaxis]
	pred_ctr_y = dy[:, np.newaxis] * heights[:, np.newaxis] + ctr_y[:, np.newaxis]
	pred_w = np.exp(dw)[:, np.newaxis] * widths[:, np.newaxis]
	pred_h = np.exp(dh)[:, np.newaxis] * heights[:, np.newaxis]
	
	pred_boxes = np.zeros(deltas.shape, dtype = deltas.dtype)
	# x1
	pred_boxes[:, 0] = (pred_ctr_x - 0.5 * pred_w).squeeze()
	# y1
	pred_boxes[:, 1] = (pred_ctr_y - 0.5 * pred_h).squeeze()
	# x2 (note: "- 1" is correct; don't be fooled by the asymmetry)
	pred_boxes[:, 2] = (pred_ctr_x + 0.5 * pred_w - 1).squeeze()
	# y2 (note: "- 1" is correct; don't be fooled by the asymmetry)
	pred_boxes[:, 3] = (pred_ctr_y + 0.5 * pred_h - 1).squeeze()
